...
| M-03 | On the host, verify the helper paths, root ownership, modes, and `sudo -l -U <username>`; run `sudo visudo -cf /etc/sudoers.d/pi-manager`; confirm only the explicit Pi Manager entry is granted. | NOT RUN |
...
| M-15 | On one disposable host, run each preview/audit/holds/failed-service action; confirm retained sensors update. Enable dangerous controls for that host only, start normal upgrade, dist-upgrade, configure, repair, autoremove, autoclean, and clean actions one at a time; confirm each returns promptly with a job, a concurrent second job is rejected, and no automatic reboot occurs. | NOT RUN |
